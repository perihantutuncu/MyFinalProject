...
namespace Business.Concrete
{
    public class ProductManager : IProductService
    {
        ...
        {
            if (ChectkIfProductCountOfCategoryCorrect(product.CategoryId).Success)
            {
        ...
        private IResult ChectkIfProductCountOfCategoryCorrect(int categoryId)
        {
            int productListCount = _productDal.GetAll(p => p.CategoryId == categoryId).Count;
            if (productListCount >= 10)
            {
        ...
        private IResult ChectkIfProductNameExists(string productName)
        {
